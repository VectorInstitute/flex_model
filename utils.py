...
def _recursively_find_first_tensor(obj: Union[InternalObject, LeafObject, ScalarObject]) -> Optional[Tensor]:
    # TODO: Deprecate
    if is_leaf_obj(obj):
        return obj
...
def _flatten(root_obj: Any) -> Tuple[Union[InternalNode, LeafNode, ScalarNode], List[Optional[Tensor]]]:
    order = []
    leaves = []
    def _dfs(obj):
        # Leaf obj case
...
def _unflatten(root_node: Union[InternalNode, LeafNode, ScalarNode], leaves: List[Optional[Tensor]]) -> Any:
    ...
    def _dfs(node):
        ...
        # Internal node case
            ...
            obj = node.unflatten(
                _dfs(child) for child in node.children
            )
            return obj
